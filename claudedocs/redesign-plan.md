...
1. **`date` callers.** `rg -n "\\.data\\.date|post\\.data\\.date|frontmatter\\.date" src/ astro.config.ts` — expected hit set: `src/utils/posts.ts`, `src/components/ListPosts.vue`, `src/pages/posts/[...slug].astro`, `src/pages/rss.xml.ts`. Any additional hits must be updated when the schema changes (Wave 1).
...
5. **Asset acquisition: Inter Tight WOFF2.** Self-hosted Inter Tight (weights 500, 600, 700; regular + italic = 6 files) is a prerequisite for Wave 1, not a code task. **Acquire and commit these files before the branch opens**, otherwise the `@font-face` declarations in `global.css` will 404 and the entire UI sans stack falls back to system fonts. Source: download the Inter Tight static fonts from the official `rsms/inter` GitHub releases (or Google Fonts → unzip → run the `.ttf` files through `woff2_compress` from the `woff2` package). Place at `public/fonts/WOFF2/Inter Tight/InterTight-Medium.woff2`, `…-MediumItalic.woff2`, `…-SemiBold.woff2`, `…-SemiBoldItalic.woff2`, `…-Bold.woff2`, `…-BoldItalic.woff2`. Confirm file presence (`ls public/fonts/WOFF2/Inter\ Tight/`) before merging Wave 1.
   - **License file (SIL OFL 1.1) must travel with the WOFF2s.** Inter Tight is licensed under the SIL Open Font License 1.1, which requires the license text be redistributed alongside the font files. Commit `OFL.txt` to `public/fonts/WOFF2/Inter Tight/OFL.txt` — copy verbatim from the upstream source (the `rsms/inter` repo ships `LICENSE.txt` at its root with the OFL text; rename to `OFL.txt` for clarity alongside the Valkyrie B family if it has its own license file, or keep the original filename — the requirement is that the text be present, not the filename). The file is ~4KB and never fetched by browsers; it exists for license-compliance audits. While here, audit `public/fonts/WOFF2/Valkyrie B*/` for an existing license file — Valkyrie B is commercial (not OFL), so the redistribution rules differ; if no license file is currently present and the foundry's terms require it, address in the same pre-flight pass. Add both license files to `.gitattributes` as `text` so they don't get treated as binary by diff tools.
...
9. **In-body image inventory.** `rg -nlE "^image:" src/content/` and `rg -nE "<img |!\\[" src/content/`. Today no post uses the schema's optional `image:` field and there are no inline body images, so the redesign can ship without an in-body image story. If that changes, post bodies must migrate from raw `<img>` to Astro's `<Image />` (via an `<MdxImage />` component mapped through `astro:assets`) to pick up format conversion and responsive `srcset`. Re-run this grep at PR time; if any hit appears, add the migration to the PR rather than punting.
   - **Pin the `image:` field's render target.** The blog schema permits `image: { src, alt }` but it's never rendered anywhere — orphaned. Two ways out:
     - **Option A (preferred, low-effort): wire it as a per-post OG override in `BaseHead.astro`.** Pass `post.data.image` from the post detail page to `BaseHead` as an optional `socialImage?: { src: string; alt: string }` prop. When present, `BaseHead` emits `<meta property="og:image" content={socialImage.src} />` (resolved to an absolute URL via `new URL(socialImage.src, Astro.site).toString()`) and `<meta property="og:image:alt" content={socialImage.alt} />` instead of the site-wide `og-image.png`. The site-wide card stays the default for every other page. This earns the schema field its keep without committing to per-post `@vercel/og` generation, and authors who care about a specific post's share preview just drop a 1200×630 PNG into `public/images/posts/` and reference it in frontmatter.
     - **Option B: delete the field from the schema** to remove the orphan. Cleanest if per-post OG overrides feel like a feature in search of a use case.
     - **Pick Option A.** The field already exists, the migration cost is ~15 lines in `BaseHead` + the post page, and the alternative ("delete now, re-add later when needed") guarantees a future schema migration that breaks every existing post's frontmatter. Keep the field, give it a job, document the size constraint (1200×630, ≤200KB) in `src/content/config.ts` as a Zod refinement comment.
10. **Trailing-slash policy — decide and pin before the rewrite.** Astro 4's `build.format` defaults to `'directory'`, which emits `dist/blog/index.html` served at the URL `/blog/`. GitHub Pages serves both `/blog` and `/blog/` for that file, but **redirects `/blog` → `/blog/` with a 301** — which costs a round-trip on every navigation, and more importantly **breaks view transitions cleanly**: the `astro:page-load` event fires on the *final* URL after the redirect, but the morph animation starts against the pre-redirect document, and on some browsers (Safari ≤17) the transition is cancelled outright when the navigation 301s mid-flight. The plan currently writes internal links without trailing slashes (`href="/blog"`, `href="/now"`, `href="/#contact"`); this will silently cost performance and morph quality on every internal click. Pick one before Wave 1 lands:
    - **Option A (recommended): keep `build.format: 'directory'`, update every internal href to trailing-slash form.** Edit `siteConfig.header.navLinks` to `'/blog/'`, `'/now/'`. Edit `siteConfig.footer.*` similarly. Edit hardcoded hrefs in `index.astro` (`<a href="/blog/">Read the writing →</a>`), `EdFooter.vue` Index column, `404.mdx` CTAs. Hash-only hrefs (`/#work`, `/#contact`) stay slash-less since the root is just `/`. Document the rule once in `siteConfig.ts` as a comment: "Internal route hrefs end in `/` to match Astro's directory build format and avoid GH Pages 301s."
    - **Option B: set `build.format: 'file'` in `astro.config.ts`.** Emits `dist/blog.html` served at `/blog` (no trailing slash, no redirect). Internal hrefs stay as written. Tradeoff: breaks any consumer that assumes directory URLs (e.g. a future serverless function that pattern-matches on path), and `/now/` becomes `/now` which loses a small "this is a section" semantic.
    - **Option C: explicit no-decision.** Accept the 301s and the view-transition risk; document it in the PR description. Not recommended — the cost compounds with every nav.
    - Pick **Option A** as the default. The grep to enforce after Wave 3: `rg -nE 'href="/(blog|now|projects|posts)[^/#"]*"' src/` should return zero hits (every internal route href ends in `/` or `#…`). Add this as a verification step.
...
The redesign uses a handful of recent platform features. Pin the support matrix once so future fallback decisions are anchored to a stated floor rather than re-litigated per feature:

- **Safari ≥ 16.4** (March 2023) — needed for `:has()`, container queries, native `scroll-behavior: smooth` honoring `prefers-reduced-motion`. View transitions land in Safari 18 (Sept 2024); on Safari 16/17 they no-op gracefully and reveals fall back to `DOMContentLoaded` per §EditorialLayout fallback path.
- **Chrome / Edge ≥ 111** (March 2023) — needed for `:has()`, full view-transition support. CSS `@supports (selector(:has(*)))` gates anything that *requires* `:has()` (the `[data-reveal]` JS-off fallback).
- **Firefox ≥ 121** (Dec 2023) — first Firefox with `:has()` enabled by default. View transitions are still behind a flag in Firefox as of early 2026; the reveal-observer fallback covers this — Firefox users see immediate reveals on initial load and the hash-scroll listener fires on `DOMContentLoaded` + `popstate` rather than `astro:page-load`. **This is the floor's weakest case** — `transition:persist` doesn't apply (no view transitions), so `EdNav` re-hydrates on every nav and `useHeaderScroll` re-attaches its listener. Acceptable degradation.
- **iOS Safari ≥ 16.4** (same as desktop Safari floor) — covers iPhone X and later running iOS 16+. Notched-device safe-area handling (§global.css iOS safe-area insets) requires `viewport-fit=cover`, supported since iOS 11.
- **No support target**: IE 11, legacy Edge (pre-Chromium), iOS Safari ≤ 15, Android Browser ≤ 4. These browsers see a degraded but readable site — fonts and palette render, but the grain overlay, view transitions, and `:has()`-gated fallbacks no-op. Don't add polyfills for them.

**Features that need conditional behavior, not graceful no-op:**
- View transitions — fallback path documented in §EditorialLayout (reveal observer + hash-scroll listen for `DOMContentLoaded` + `popstate`).
- `forced-colors` — handled via `@media (forced-colors: active)` block in `global.css`; covers Windows High Contrast and assistive tech regardless of browser version.
- `prefers-reduced-motion` — universal support across the floor; no fallback needed.

If a future feature needs a polyfill or a more conservative implementation, the decision is "does it pass the floor?" rather than "does it pass every browser?" — the latter is unbounded.
...
- **Motion tokens** — colors got centralized; motion has to as well, or five components hardcoding `transition: … 200ms ease` will drift the moment a single duration changes. Add to `:root`:
  ```css
  --dur-fast: 120ms;        /* micro: theme-toggle icon, focus ring */
  --dur-base: 200ms;        /* default: hover color, pill bg, theme cross-fade */
  --dur-slow: 360ms;        /* deliberate: drawer slide, reveal observer fade-in */
  --dur-marquee: 60s;       /* wire-marquee loop */
  --ease-standard: cubic-bezier(0.2, 0, 0.2, 1);     /* default for hover/state */
  --ease-emphasized: cubic-bezier(0.2, 0, 0, 1);     /* drawer, reveal */
  --ease-linear: linear;                              /* marquee only */
  ```
  Every `transition` / `animation` in the new component CSS reads from these vars (e.g. `transition: color var(--dur-base) var(--ease-standard)`). No raw `200ms ease` literals in component styles.
- **Z-index scale** — at least seven stacking layers across six files (skip-link, drawer + backdrop, topbar, nav, side rails, ScrollToTop, shell). Centralize:
  ```css
  --z-base: 0;
  --z-grain: 1;             /* body::before paper-grain overlay */
  --z-shell: 2;             /* page content */
  --z-side-rails: 10;       /* fixed left/right strips */
  --z-scroll-top: 40;
  --z-topbar: 50;           /* sticky */
  --z-nav: 60;              /* sticky, sits above topbar at scroll */
  --z-drawer-backdrop: 80;
  --z-drawer: 81;
  --z-skip-link: 100;       /* always wins */
  ```
  Every fixed/sticky/absolute element references one of these. No raw `z-index: 100` literals.
- **Spacing & type scale** — `tokens.css` should hold at least the hero-scale clamp values and a small spacing rhythm so future authors don't reinvent them. Add:
  ```css
  /* fluid type — used by hero H1 and editorial section H2 */
  --fs-display: clamp(48px, 7.6vw, 112px);
  --fs-h1: clamp(36px, 5vw, 64px);
  --fs-h2: clamp(28px, 3.4vw, 44px);
  --fs-h3: 22px;
  --fs-body: 17px;          /* prose body */
  --fs-ui:   15px;          /* nav, buttons */
  --fs-caps: 10.5px;        /* eyebrows, coords */
  /* spacing rhythm — section padding, hero gutter */
  --space-section: clamp(64px, 9vw, 128px);
  --space-block: clamp(32px, 4vw, 56px);
  --space-row: 24px;
  ```
  Component CSS reads `padding-block: var(--space-section)` rather than re-deriving clamps. UnoCSS utilities still work for one-offs.
...
- **Add native smooth-scroll, gated on reduced-motion.** `useHashScroll` only intercepts *clicks*; native hash navigation (typing `/#work` in the URL, browser back/forward to a hash, server-rendered hash anchors) is jumpy without this. Add: `@media (prefers-reduced-motion: no-preference) { html { scroll-behavior: smooth; } }`. The media-query wrap is required — a global `scroll-behavior: smooth` overrides user reduced-motion preferences on browsers that don't auto-disable it (Safari ≤16 ignored the auto-disable).
...
- **JS-off fallback for `[data-reveal]`.** The reveal CSS hides every `[data-reveal]` element until the IntersectionObserver sets `data-revealed` on it. With JS disabled, on a slow connection that delays observer registration, or if the inline script throws, every reveal-gated section stays permanently invisible — which is most of the homepage below the hero. Two mitigations, ship both: (a) in `global.css`, add `[data-reveal]:not([data-revealed]) { opacity: 1; transform: none; }` *inside* a `@supports (selector(:has(*)))` wrapper that is then **overridden** by a `.js [data-reveal]:not([data-revealed]) { opacity: 0; transform: translateY(8px); }` rule; (b) in `BaseHead.astro`, the very first line of the inline no-flash script does `document.documentElement.classList.add('js')`. The class lands before first paint, so JS-enabled clients see the hidden state and the observer reveals normally, while JS-disabled clients (where the class never lands) see content immediately. Simpler alternative if the `:has()`/`@supports` dance reads as over-engineered: `<noscript><style>[data-reveal]{opacity:1!important;transform:none!important}</style></noscript>` in `BaseHead` — works for the disabled-JS case but not for the "observer registered late" case. Pick the `.js` class approach; it covers both.
...
- **Heading anchor affordance** (depends on the `rehype-autolink-headings` wiring above). The plugin appends `<a class="heading-anchor" aria-label="Permalink to this heading">¶</a>` to each `h2`–`h4`. Style as: `position: relative` on the heading; `.heading-anchor { opacity: 0; margin-left: 0.4em; color: var(--ink-faint); font-family: var(--mono); font-size: 0.7em; vertical-align: middle; text-decoration: none; transition: opacity var(--dur-base) var(--ease-standard), color var(--dur-base) var(--ease-standard); }` then `@media (hover: hover) { h2:hover .heading-anchor, h3:hover .heading-anchor, h4:hover .heading-anchor, .heading-anchor:focus-visible { opacity: 1; color: var(--coral); } }`. Always visible (`opacity: 1`) on touch — `@media (hover: none) { .heading-anchor { opacity: 0.5; } }` — since there's no hover state to discover them.
- **Footnotes.** Markdown footnotes render as `<sup><a href="#fn-1" id="fnref-1">1</a></sup>` inline and a trailing `<section class="footnotes"><ol>…</ol></section>`. Style: superscript refs get `color: var(--coral); font-family: var(--mono); font-size: 0.65em; font-weight: 600; padding: 0 2px; text-decoration: none;` with a hairline coral underline on hover; the footnotes section gets `margin-top: 3em; padding-top: 1.5em; border-top: 1px solid var(--line); font-size: 0.875em; color: var(--ink-soft);` and the back-reference arrow (`↩`) is `color: var(--coral); text-decoration: none; margin-left: 0.25em;`.
- **Figures with `<figcaption>`.** Body images authored via the `<MdxImage />` component wrap in `<figure>`. Style: `figure { margin: 2em 0; } figure img { width: 100%; height: auto; display: block; } figcaption { font-family: var(--caps); font-size: var(--fs-caps); letter-spacing: 0.18em; text-transform: uppercase; color: var(--ink-faint); margin-top: 0.75em; padding-left: 1em; border-left: 2px solid var(--coral); }`. Matches the editorial dispatch-table treatment.
- **Task lists** (GFM). `.prose ul.contains-task-list { list-style: none; padding-left: 0; } .prose .task-list-item { padding-left: 0; } .prose .task-list-item input[type="checkbox"] { appearance: none; width: 14px; height: 14px; border: 1px solid var(--ink); border-radius: 2px; margin-right: 0.5em; vertical-align: middle; background: var(--paper); } .prose .task-list-item input[type="checkbox"]:checked { background: var(--coral); border-color: var(--coral); }`. No JS — pure CSS.
- **Definition lists.** `.prose dl { margin: 1.5em 0; } .prose dt { font-family: var(--caps); font-size: var(--fs-caps); letter-spacing: 0.18em; text-transform: uppercase; color: var(--ink-faint); margin-top: 1em; } .prose dd { margin-left: 0; padding-left: 1em; border-left: 1px solid var(--line); color: var(--ink-soft); }`.
- **Drop cap — opt-in only.** Editorial framing earns a drop cap, but applied universally to every post's first paragraph it competes with the existing serif H1 + roman §. Make it an opt-in class authors add to the first paragraph of long-form essays: `.prose .lede::first-letter, .prose p.lede:first-of-type::first-letter { float: left; font-family: var(--serif); font-weight: 500; font-size: 4.5em; line-height: 0.85; padding: 0.05em 0.12em 0 0; color: var(--coral); font-style: italic; }`. Authors opt in with `<p class="lede">…</p>` in MDX (or via a `<Lede>` MDX shortcut if desired later). Not auto-applied. Document the rule in `prose.css` comments.

### Syntax highlighting theme (Shiki) — `astro.config.ts`

**Audit first.** `astro.config.ts` already configures `shikiConfig.themes: { light: 'github-light-default', dark: 'github-dark-default' }` with `wrap: true`. The `-default` variants are GitHub's current canonical themes (Shiki ≥1.0). Decision: **keep `github-*-default`** — they read acceptably on `--bone` (light) and the Ink code background (dark). Do **not** silently swap to `github-light` / `github-dark-dimmed` unless §Verification step 21 flags a contrast or hue clash.

`prose.css` retokenizes the `pre`/`code` *container*; Shiki paints the tokens inside. If `github-*-default` ever looks foreign on the Paper/Ink palette, swap to `vitesse-light` / `vitesse-dark` or define a custom Shiki theme JSON that uses `--ink` for plain text, `--coral` for keywords, and `--olive` for strings. Verify by building and opening `/projects/dotfiles` (the only post with a fenced code block today; if none exist when the rewrite lands, add a throwaway markdown fixture in `claudedocs/` and render it locally).

### Markdown plugins (Wave 1) — `astro.config.ts`

**Audit reveals a false assertion in the original plan: `rehype-autolink-headings` is NOT currently wired.** `astro.config.ts:24-32` declares only `shikiConfig` under `markdown` — no `rehypePlugins`, no `remarkPlugins`. The §Post detail claim that anchors are "already supplying slugs" is wrong as of today. Wire them in this wave so the TOC removal doesn't leave long posts with zero in-prose navigation:
...
// ...
markdown: {
  shikiConfig: { themes: { light: 'github-light-default', dark: 'github-dark-default' }, wrap: true },
  rehypePlugins: [
    rehypeSlug,
    [rehypeAutolinkHeadings, {
      behavior: 'append',
      properties: { className: ['heading-anchor'], ariaLabel: 'Permalink to this heading' },
      content: { type: 'text', value: '¶' },   // pilcrow renders well in serif
    }],
  ],
},
```

`npm i -D rehype-slug rehype-autolink-headings` adds two small deps (~3KB compressed combined). Style the appended `.heading-anchor` in `prose.css` per the "Heading anchor affordance" rule below.
...
- **Analytics re-fire on view transitions.** Mounting `<ViewTransitions />` (required for reveals and hash scroll, see §EditorialLayout) silently breaks pageview counting for every common analytics provider — internal navigations no longer trigger a full document load, so the provider's load-time hook never re-fires. Result post-deploy: pageviews drop ~40–60% (only hard loads counted), bounce rate spikes, time-on-site metrics warp. Wire the re-fire **in the same commit as the chrome merge**, not as a follow-up. Patterns by provider (audit which one the existing snippet uses, then apply the matching pattern):
  - **Plausible** (most likely candidate for a personal site): script tag stays inline; add a sibling inline script `document.addEventListener('astro:page-load', () => { if (window.plausible) window.plausible('pageview') })`. Plausible's auto-tracking only fires on initial load when mounted this way; the explicit call covers SPA-style navs.
  - **GA4 (`gtag.js`)**: add `document.addEventListener('astro:page-load', () => { if (window.gtag) gtag('event', 'page_view', { page_path: location.pathname + location.search, page_title: document.title }) })`. Note: `document.title` is updated by Astro before `astro:page-load` fires, so the destination title is correct.
  - **Fathom**: `document.addEventListener('astro:page-load', () => { if (window.fathom) fathom.trackPageview() })`.
  - **Self-hosted / Umami / other**: locate the provider's "manually track pageview" method; if none exists, dispatch a synthetic `PageTransitionEvent` or pseudo-`load` and accept the limitation.
  - **None of the above (no analytics today)**: skip the re-fire entirely and note in the PR description that any future analytics wire-up must include the `astro:page-load` hook.
  Verify post-deploy: open the analytics dashboard's realtime view, navigate `/` → `/blog` → a post detail → back to `/` (all via internal clicks, no hard refresh), and confirm four distinct pageviews registered, not one. If only the hard-load pageview shows, the re-fire is missing or the inline script is being CSP-blocked (see §"CSP / inline-script audit" below).
- **CSP / inline-script audit (do before Wave 1 lands).** The redesign introduces **four inline `<script>` blocks** in `BaseHead.astro`: (1) the `.js` class-flag + no-flash theme script, (2) the `astro:page-load` reveal observer, (3) the `schema.org/Person` JSON-LD, (4) the conditional `schema.org/BlogPosting` JSON-LD on post detail. Plus whatever the existing analytics snippet adds (commonly a 5th inline block). **Audit for a Content-Security-Policy *before* the rewrite lands**, since CSP failures are silent-from-the-author's-perspective (the page paints with no theme, no reveals, no rich-results — exactly the symptoms that look like "the rewrite is broken" rather than "policy is blocking scripts"). Steps: (a) `rg -nE "Content-Security-Policy|http-equiv=.csp." src public astro.config.ts` and `ls public/_headers public/.htaccess 2>/dev/null` — confirm no CSP exists today (GitHub Pages doesn't add one by default, so the expected result is zero hits). (b) If a CSP is found, the four inline blocks need either `nonce` attributes (Astro 4 supports this via `<script is:inline nonce={Astro.locals.cspNonce}>` plus middleware that emits a per-request nonce header — but GH Pages is static so per-request nonces aren't possible; the realistic options are `'unsafe-inline'` for `script-src` or sha256 hashes of each inline block) or migration to external files in `public/scripts/`. (c) If no CSP exists, **note explicitly in the PR description** that the redesign assumes no CSP and that adding one later requires reworking these blocks — so a future security-pass doesn't silently break the site. (d) Independent of CSP, the inline JSON-LD blocks are safe to externalize if desired (move to `public/structured-data/person.json` linked via `<link rel="alternate" type="application/ld+json" href="…">` — but most validators prefer inline, so default to inline).
...
- **Update the inline no-flash theme script.** Today's script almost certainly sets `data-theme` on `<body>` or `<html>`. With Ink wired to `html.dark` (per the decision above), the script must read `localStorage` / `prefers-color-scheme` and set `document.documentElement.classList.toggle('dark', isDark)` *before* first paint. Without this change the page paints in Paper, then snaps to Ink after `ThemeToggle` hydrates.
  - **Pin the storage key literal: `'vueuse-color-scheme'`.** This is `@vueuse/core`'s default for `useDark()` and the inline script must read the same key, with the same value semantics (`'dark' | 'light' | 'auto'` — when `'auto'` or unset, fall through to `window.matchMedia('(prefers-color-scheme: dark)').matches`). Hardcode the literal in the inline script *and* call `useDark({ storageKey: 'vueuse-color-scheme' })` explicitly in `ThemeToggle.vue` even though it's the default — the explicit call documents the contract and prevents a future `@vueuse` major-version default change from silently breaking no-flash. The inline script body:
    ```js
    (function () {
      try {
        ...
        document.documentElement.classList.add('js');   // see §global.css [data-reveal] JS-off fallback
      } catch (e) { /* storage blocked (Safari private mode, etc.) — fall through to Paper */ }
    })();
    ```
  Wrap the body in a named function and re-invoke on `astro:after-swap` so theme state survives view transitions (the inline `<script>` tag is replaced during transitions; without re-invocation the `html.dark` class is preserved by `transition:persist` on the root but the function definition is gone — important if a future page hook needs to re-evaluate).
...
Then `WriteItem.vue` reads `post.data.dateRaw.getUTCFullYear()` and `.toLocaleString('en', { month: 'short', timeZone: 'UTC' }).toUpperCase()`, while all existing callers of `post.data.date` continue to work untouched. `rss.xml.ts` should switch to `dateRaw` for the proper RFC-822 `<pubDate>` (today it ingests the formatted string, which is technically invalid RSS — fix in the same commit).

**Pin `timeZone: 'UTC'` everywhere `dateRaw` renders to a human string.** Without this, `Date.prototype.toLocaleString` and `.getFullYear()` run in the build server's local timezone — a post dated `2025-01-01` (which Zod coerces to `2025-01-01T00:00:00.000Z`) will render as `DEC 2024` if CI runs in `America/Los_Angeles`. Convention for the redesign: blog dates are *calendar dates* in UTC, not points in time, and every consumer must read them as such. Pin in three places: (a) `formatDate()` in `src/utils/posts.ts` — verify the existing implementation already passes `{ timeZone: 'UTC' }` to `toLocaleString` (audit before Wave 1; if it doesn't, fix in the same commit as the schema migration since the visible "Jan 15, 2025" string will silently drift one day on west-coast builds); (b) `WriteItem.vue` month/year derivation as above; (c) `rss.xml.ts` `<pubDate>` — `dateRaw.toUTCString()` is correct for RFC-822 and timezone-stable. Document the rule once at the top of `src/utils/posts.ts` so the next consumer doesn't reinvent the bug.

**Run `npx astro sync` immediately after touching `src/content/config.ts`.** Astro generates `.astro/types.d.ts` from the content schemas, and editors / `tsc --noEmit` / `astro check` rely on it. Without a `sync`, the first build after the schema change will surface TypeScript errors that look like real bugs ("Property `dateRaw` does not exist on type…", "Property `status` does not exist on `ProjectsEntry`") in every consumer — `WriteItem.vue`, `WorkCard.vue`, `rss.xml.ts`, `index.astro`, `sortProjects`, `EditorialLayout`'s badge counts. The fix is one command, but mistaking the symptom for a code bug can waste 20 minutes mid-merge. Sequence: edit `src/content/config.ts` → `npx astro sync` → commit *both* the schema edit and any `.astro/` artifacts → re-run `astro check` to confirm green. `astro dev` re-runs sync automatically on schema changes, so day-to-day local development is fine; the explicit `sync` is for the CI / cold-build path where `dev` hasn't been invoked. Add an npm script `"sync": "astro sync"` if it doesn't already exist for muscle-memory's sake.

For the `projects` schema extension, add **five** optional fields with defaults: `stack: z.array(z.string()).optional()`, `status: z.enum(['active','archive']).default('active')`, `category: z.string().optional()`, **`order: z.number().optional()`**, and **`link: z.object({ href: z.string().url(), label: z.string().optional() }).optional()`** (consumed by the project detail header's primary link button — see §Project detail). The `order` field is the canonical sort key — without it the homepage Selected Work `slice(0, 8)` is non-deterministic on rebuilds (file-system iteration order is not stable across platforms). Sort rule used everywhere projects are rendered: `(a, b) => statusRank(a) - statusRank(b) || (a.data.order ?? 999) - (b.data.order ?? 999) || a.slug.localeCompare(b.slug)` — where `statusRank` returns `0` for `'active'`, `1` for `'archive'`. Backfill `order` on existing entries (1, 2, 3, …) in the same commit as the schema migration. Entries without `order` sort to the end alphabetically — deterministic, but the intent is that every published project gets an explicit number.
...
- **`src/components/WorkCard.vue`** — single work-card row. Props: `idx` (string, e.g. `'01'`), `category`, `name` (may contain italic emphasis HTML), `stack?`, `blurb`, `status` (`'active' | 'archive'`), `href`. Renders prototype 753-895 row markup. Reused by homepage Selected Work *and* `/projects` index — single component, no duplication. Cards must render gracefully when `stack` or `category` is missing (omit the row rather than render an empty cell).
  - **Archive vs active visual treatment** (otherwise the `status` prop is decorative and the sort key sorts to nothing visible). Active: as specced. Archive: (a) the entire card gets `opacity: 0.62` and a 1px `--line-soft` left rule replaces the default `--line`; (b) `name` color stays `--ink` (don't lose legibility) but italic-emphasis spans inside `name` drop the coral hue → `color: var(--ink-mute)` so the card reads as quieted; (c) the `idx` cell prefixes the number with a coral middot caret (`·`) instead of standalone — `<span class="idx">{idx}<small class="idx-tag">arch</small></span>` where `.idx-tag` is `var(--caps) 9px tracking-0.2em color var(--ink-faint)` set below the index number; (d) hover state is *not* suppressed (archive cards are still navigable) but the hover opacity rises to `0.85` rather than `1` so the quieting persists. Test by setting one project to `status: 'archive'` and verifying it visually steps back from the active siblings in the same grid.
...
export interface WriteItemProps {
  ...
  tags: string[];                        // first 3 rendered; see schema-vs-prop note below
  href: string;
}
...
export interface EdNavProps {
  ...
  writingCount?: number;
  // NOTE: no `currentPath` prop — EdNav persists across view transitions
  // (transition:persist) and would freeze a prop value. Reads window.location
  // reactively from a local ref instead. See §EditorialLayout "Persisted-island
  // prop staleness". The badge counts above are tolerable as props because
  // they're stable for any given build (content size doesn't change between
  // client-side navs).
}
...
**Schema-vs-prop conversion for `WriteItem.tags`.** The blog schema declares `tag?: string` (single optional category, e.g. `'Notes'`), but `WriteItemProps.tags` is `string[]` with a 3-item slice — the array shape leaves room to widen the schema later without re-typing the component. The conversion lives at every call site as a one-liner: `tags={post.data.tag ? [post.data.tag] : []}`. Pin this in three places so it isn't reinvented:

- Homepage `index.astro` §Writing, where it maps `getPosts().slice(0, 2)` to `<WriteItem />`.
- `src/pages/blog/[...path].astro`, where the rebuilt `ListPosts.vue` (now a thin wrapper, see below) maps the filtered post list.
- `ListPosts.vue` itself if it survives as a wrapper — though §Cleanup deletes it, so the conversion only needs to live at the two `.astro` call sites.

If a future migration switches `tag: string` → `tags: z.array(z.string()).optional()`, the call-site lines become `tags={post.data.tags ?? []}` and `WriteItem` is untouched. Do **not** change the schema in this PR; the single-tag taxonomy is acknowledged as sparse in §"Features explicitly NOT in scope".
...
- **`lang?: string`** — emitted on the root `<html>` element. Default `'en-US'`. Post detail pages pass `post.data.lang` so multilingual posts get the right `<html lang>` for screen-reader pronunciation and translation tooling. Today `BaseLayout.astro:17` hardcodes `<html lang="en">`; this prop replaces that pattern.
- **`noindex?: boolean`** — when `true`, `EditorialLayout` forwards to `BaseHead` which emits `<meta name="robots" content="noindex,nofollow" />`. Default `false`. `/404` sets `noindex={true}`; future under-construction or thank-you pages do the same. Also forward `socialImage?: { src: string; alt: string }` and `ogType?: 'website' | 'article'` for the OG meta-tag set in §"OG / social card".

**`<noscript>` handling for `ThemeToggle`.** With JS disabled, the inline no-flash script still sets `html.dark` correctly on first paint, but the `<ThemeToggle />` button renders as an inert pill. Add to `BaseHead` (or `global.css`): `<noscript><style>.theme-toggle { display: none !important; }</style></noscript>`. Cheaper than a server-rendered fallback toggle, and the no-flash script picks the right theme without user interaction anyway.
...
**`transition:persist` on every Vue island.** With `<ViewTransitions />` mounted, Astro **tears down and re-mounts every island on every internal navigation by default** — that means `EdNav`, `NavDrawer`, `ThemeToggle`, and `ScrollToTop` re-hydrate on every `/` → `/blog` click. Visible consequences: (a) `useHeaderScroll` re-attaches its scroll listener mid-swap, briefly losing the hide-state; (b) the drawer's `isOpen` state is wiped if the user navigated from a deep link with the drawer in some state; (c) the brand-mark `view-transition-name: brand-mark` morph fights the island re-mount (the morph runs against an unmounted-then-remounted DOM node); (d) `client:idle` re-fires its idle-callback wait, which is wasted work since the components were already hydrated. **Add `transition:persist` to every Vue island in `EditorialLayout.astro` markup**: `<EdNav transition:persist client:idle />`, `<ScrollToTop transition:persist client:idle />`. The drawer trigger is inside `EdNav` so it persists transitively. `ThemeToggle` is also inside `EdNav` — same. `EdFooter` and `EdSideRails` and `EdTopbar` are static Astro components (no hydration) and don't need the directive; only `client:*`-hydrated components do. Caveat: `transition:persist` requires a stable `transition:persist="<id>"` name when the same component appears on both pages but in different DOM positions — in this layout the islands sit at identical DOM positions on every page, so the default (auto-generated id) is fine; do **not** supply explicit ids unless a future page rearranges the chrome. Verify after Wave 2: open devtools → Components panel → navigate `/` → `/blog` and confirm the `EdNav` Vue instance is the *same* instance (state inspector shows preserved reactive refs), not a new one.

**Persisted-island prop staleness — `currentPath` cannot be a prop.** A persisted Vue island captures its props at first mount; subsequent navigations do **not** re-pass props to the same instance. Passing `currentPath={Astro.url.pathname}` from `EditorialLayout` to a persisted `EdNav` therefore freezes `aria-current="page"` on whichever link was current at first paint — "Writing" stays highlighted forever after you nav from `/blog` to `/`. The two rules (persist + path-as-prop) collide; pick one resolution and apply it consistently:

- **Resolution (chosen): read path reactively inside `EdNav` from `window.location`, not from a prop.** `EdNav.vue` initializes `const currentPath = ref(typeof window !== 'undefined' ? window.location.pathname : '/')` and subscribes once on mount: `document.addEventListener('astro:after-swap', () => { currentPath.value = window.location.pathname })`. The same pattern lives in `NavDrawer.vue` (inside `EdNav`, so it inherits). On SSG (no `window`), the initial render falls back to `'/'` — fine, because the ref hydrates to the real path before first paint via `astro:page-load` (which fires on initial load too).
- **Drop the `currentPath` prop from `EdNavProps` and `NavDrawerProps`.** `EditorialLayout` passes no path; the prop-types block in §"Prop type contracts" omits it. The `EdNav` matching rule (exact for static routes, prefix for `/blog/*` and `/posts/*`, no match for hash-only links) is unchanged — just reads from the local ref.
- **Re-verification in §Verification step 8:** after navigating `/` → `/blog`, `aria-current="page"` must move from no-match (homepage anchor) to the Writing link. After navigating back, it must clear. If the `after-swap` listener isn't wired, the highlight will be stuck — that's the symptom to grep for.

Same constraint applies to **nav badge counts**: `workCount` / `writingCount` are passed as props once at first mount and would freeze identically. Acceptable for this redesign because the counts are stable for any given build (content collection size doesn't change between client-side navs — only between deploys). Document the assumption inline in `EditorialLayout` so a future "live data" experiment doesn't reintroduce the bug.
...
- **`aria-current="page"` on the active nav link.** Without it, screen-reader users have no signal that "Writing" is the current section while browsing `/blog`. `EdNav` reads `currentPath` **from a reactive local ref**, not from a prop, because the island persists across view transitions and would freeze any prop value (see §EditorialLayout "Persisted-island prop staleness"). Init: `const currentPath = ref(typeof window !== 'undefined' ? window.location.pathname : '/')`; refresh on `astro:after-swap`. Apply `aria-current="page"` to the link whose route matches. Matching rule: exact match on the link's `href` for static routes (`/now/`), prefix match for collection roots (`/blog/` matches any `/blog/…` or `/posts/…` URL; `/#work` matches when `currentPath === '/'`, since the homepage anchor is the canonical "Work" target). The Contact link (`/#contact`) never sets `aria-current` — it's an anchor, not a route. Pair with a CSS hook: `.nav-link[aria-current="page"] { color: var(--coral); border-bottom: 1px solid var(--coral); }` so the active state is visible to sighted users too — currently the design has no active-link affordance, which is a navigability gap independent of a11y. `NavDrawer.vue` reads the same ref (or receives it via `provide`/`inject` from `EdNav`) and applies the same matching rule to its serif drawer items.
...
- Bottom strip: `© 2026 Brennon Williams · MIT · Made on Earth` and `Vol. 01 · Issue Nº 03 · Last build {hash}`. **Wire the build hash via Vite `define`** in `astro.config.ts`:
  ```js
...
1. **Hero** (`#top`) — exact prototype layout 605-662. Label renders `{siteConfig.editorial.series}` (do **not** hardcode "Notes from the workshop · Nº 03" in `index.astro` — see §`site-config.ts` `editorial.series`); H1 "I wonder what the **machines** think." (italic coral); lead paragraph from prototype 610; **two CTA buttons (copy + hrefs pinned, not TBD):** primary `<a class="btn-primary" href="/blog">Read the writing →</a>`; ghost `<a class="btn-ghost" href={`mailto:${siteConfig.email}`}>Get in touch</a>`. Lift glyphs (`→`) and tracking from prototype 612-617; do not parameterize copy through `siteConfig` for v1 (CTA copy is an editorial decision, not a config knob — if it ever needs to change, edit `index.astro` directly). Then: stats ring (3 cells); foot row with the decorative caps strip + `{siteConfig.editorial.coords}` (no hardcoded lat/long — pulled from config); right side: portrait plate using Astro `<Image />` (`import headshot from '~/assets/images/profile/brennon-stylized-headshot-2025.png'`) with corner brackets + four annotation tags + 4-cell index strip. Use `<Image src={headshot} alt="Portrait of Brennon Williams" widths={[480, 720, 1080]} sizes="(max-width: 820px) 90vw, 480px" loading="eager" fetchpriority="high" />` — do **not** copy the file into `public/`; the asset must flow through Astro's image optimizer (already wired in `astro.config.ts` via the `sharp` service — preserve that, don't switch to `passthroughImageService`). **The `loading="eager"` + `fetchpriority="high"` pair is required, not optional**: the portrait is the LCP element on `/` and Astro's default `loading="lazy"` measurably degrades LCP on home (typically pushes it past 2.5s on 4G). Verify in §Verification with a Lighthouse run. **Add `style="view-transition-name: hero-portrait"`** so navigations between `/` and inner pages morph the portrait if a destination ever opts in (cheap polish, harmless when no destination claims the name).
   - **Hero portrait in Ink mode — decision: single image with `mix-blend-mode: luminosity` in `html.dark`, no second asset.** The stylized headshot is a B&W-toned PNG; in Ink the default `mix-blend-mode: multiply` (inherited from the surrounding plate styling) reads muddy because the dark plate background swallows the midtones. `mix-blend-mode: screen` (suggested in the addendum) over-brightens the highlights and clips. `luminosity` preserves the portrait's tonal range while letting the plate's coral corner brackets and caps annotations show through. Implementation: `.hero-portrait img { mix-blend-mode: normal; } html.dark .hero-portrait img { mix-blend-mode: luminosity; }`. Do **not** ship a second `brennon-stylized-headshot-2025-ink.png` for dark mode — one optimized asset, one blend-mode swap, no duplication. Verify in §Verification step 3 with a side-by-side Paper/Ink screenshot.
...
3. **About** (`#about`) — Section header §I + 2-col grid: serif prose paragraphs + dispatch table aside. Prototype 694-737.
   - **Dispatch table source.** The right-column aside (rendered as a small `<dl>`/`<table>` of label→value rows, e.g. `Filed · Code · Intelligence`, `Location · Berlin, VA`, `Last build · {hash}`) **pulls values from `siteConfig.editorial`**, not hardcoded prototype copy. Specifically: `Filed` reads `siteConfig.editorial.filedUnder`, `Location` reads `siteConfig.editorial.location`, `Coordinates` reads `siteConfig.editorial.coords`, `Last build` reads `{__BUILD_HASH__}` (the Vite define from §EdFooter). The row labels themselves (`FILED`, `LOCATION`, `COORDINATES`, `LAST BUILD`) are static — they're typographic captions, not content. Pin this so the same six strings aren't redeclared between the topbar, the dispatch table, and the footer colophon — one edit to `siteConfig.editorial` propagates everywhere. Add any new aside row (e.g. `Issue · Vol. 01 · Nº 03` reading `${siteConfig.editorial.volume} · ${siteConfig.editorial.issue}`) to the table by extending the local `<dl>` markup in `index.astro`; do **not** add a new field to `siteConfig.editorial` just for this aside if the data already exists under another key.
...
- `ListPosts.vue` rebuild — drop the year-divider hero text and map over `<WriteItem />` (the same component used on the homepage). Mono `y` cell handled inside `WriteItem`. (Per §Cleanup, `ListPosts.vue` is deleted; the `.astro` page maps `<WriteItem />` directly with the tag-conversion line documented in §"Schema-vs-prop conversion".)
- **Empty / sparse-state guards for category routes.** The `[...path]` route serves `/blog/`, `/blog/writing/`, `/blog/notes/`, and `/blog/talks/`. Today only `writing/` has posts; `notes/` and `talks/` are configured-but-empty directories. Without guards, those routes render the section header followed by an empty grid — reads as broken, looks the same as a build failure. Rules:
  - If `posts.length === 0` for the current path, replace the grid with a single italic serif line inside the grid container: `"No {categoryName} published yet — drafts in flight."` where `categoryName` derives from the path (`'notes'`, `'talks'`, `'writing'`) or falls back to `'posts'` for the root `/blog/`. Use `coord` styling on a small caps eyebrow above (`Empty for now`) so the page still reads as intentional editorial space, not error state.
  - Do **not** hide the section header on empty category routes (unlike the homepage Writing section, where the entire section is suppressed). The page exists as a navigable route from the nav badge counts and the footer — the header confirms "yes, this is the right place, just nothing here yet." Hiding it on a directly-navigated URL would feel broken.
  - If `posts.length === 1`, render the single `<WriteItem />` at normal grid width (don't span columns) — the blog index is a list view, not a marquee. Different rule from the homepage Writing section where 1 post spans for visual balance.
  - Nav badge counts (from `EditorialLayout`) reflect the global writing count, not per-category — `/blog/notes/` shows the same `Writing 09` badge as `/blog/`. This is correct: the badge represents the section's total scope, not the current sub-route's filtered count.

### Post detail: `src/pages/posts/[...slug].astro`

- Wrap in `EditorialLayout` (`mainWidth="prose" lang={post.data.lang}`). Schema default is `'en-US'`; multilingual posts get the right `<html lang>` for free.
...
- **Description fallback.** `post.data.description` is optional in the schema. **Omit the italic serif description element entirely when absent** — do not render an empty `<p>` (collapses the H1→meta rhythm by ~1.5em of blank space) and do not fall back to a slice of the post body (drops half-sentences that read worse than nothing). Implementation: wrap the description render in `{post.data.description && <p class="lede-italic">{post.data.description}</p>}`. The class name avoids collision with `.lede` (drop-cap opt-in). Verify in §Verification: open a post without `description:` set; the H1 sits directly above the mono meta row with no empty gap. Same rule applies to the project detail page header when `project.data.description` is absent.
...
- **Prev/Next strip at article foot.** Without it readers hit the end with no path forward except scroll-up. Add a footer-of-article strip outside `.prose` but inside `mainWidth="prose"`: two-column grid (`grid-template-columns: 1fr 1fr; gap: var(--space-row);`) with `← Older` on the left and `Newer →` on the right. Each cell renders an eyebrow caps label (`OLDER` / `NEWER`) + serif italic title clamped to 2 lines. Source from `getPosts()` sorted by date desc; `prev = posts[currentIndex + 1]`, `next = posts[currentIndex - 1]`. If `currentIndex === 0`, omit the `Newer` cell (render an empty column, not "nothing here"); same for the oldest post and `Older`. Style as `border-top: 1px solid var(--line); margin-top: 3em; padding-top: 1.5em;` so it reads as part of the article's foot, not as page chrome. Hover: cell background flips to `var(--bone)` per `container-link` semantics. Compute prev/next in the page's frontmatter, pass as props to a small `<ArticleNav prev={prev} next={next} />` Astro component (reusable on project detail too).
...
- **Active/Archive visual break.** The sort puts every active card before every archive card, but they share one grid — without a break the opacity gradient mid-page reads as "the cards faded for no reason". Insert a sub-header *only* when both groups are non-empty: iterate the sorted list, and the first time `status === 'archive'` is encountered, emit a full-width `<div class="work-archive-divider">` row before the card. Markup: a hairline `border-top: 1px solid var(--line-soft)` strip with a left-aligned caps eyebrow reading `Archive — completed & shelved` (`eyebrow` shortcut), padded `margin: 2rem 0 0.5rem` so it visually separates the two groups without breaking the grid rhythm. The divider spans all columns (`grid-column: 1 / -1`). Suppress the divider when `projects.every(p => p.status === 'active')` or `projects.every(p => p.status === 'archive')` — a divider with one group above and nothing below (or vice versa) reads as broken. No equivalent break on the homepage Selected Work (only 8 cards; if archive entries slip into the top 8, the opacity treatment is sufficient signal).

### Project detail: `src/pages/projects/[...slug].astro`

- Wrap in `EditorialLayout` (`mainWidth="prose"`).
- Header (top to bottom): roman §, caps eyebrow rendering `{project.data.category ?? '—'}`, serif H1 (`project.data.title`, may contain `<em>` for coral italic), italic serif description if present (same fallback rule as Post detail — omit element when absent, do not fall back to body slice), then **stack mono row** + **status pill** + **primary link button** as a horizontal cluster (wraps to stack on `< 640px`).
- **Stack mono row format.** Render `project.data.stack` (added in Wave 1 schema migration) as a single line of mono caps separated by middots, prefixed with a caps label: `STACK — A · B · C · D` where the label uses `eyebrow` styling and the items use `coord` styling. No comma separators (commas read as prose, middots read as data). Omit the entire row when `stack` is absent or empty — do not render a bare `STACK —` label with no items. If `stack.length > 6`, render the first 6 followed by ` · +N more` (no link target; the row is a fact-strip, not navigation).
- **Status pill.** Always rendered (even for `'active'`, the default). Active: `pill` shortcut with text `In progress` in `--coral` text + `--coral` border. Archive: same `pill` shape with text `Archive`, `--ink-faint` text + `--line-soft` border. Sized to `font-size: 10.5px`, sits inline with the link button. Without an "active" affordance the pill exists only for archive projects, which makes its absence feel like a missing element on active project pages — render in both states.
- **Primary link button.** Source: a new optional `link` field added to the project schema in Wave 1 alongside `stack`/`status`/`category`/`order`. Shape: `link: z.object({ href: z.string().url(), label: z.string().optional() }).optional()`. Render as `btn-primary` with copy `{project.data.link.label ?? 'Visit project →'}` and `target="_blank" rel="noopener"` (external by definition — the field carries a fully-qualified URL). If `link` is absent, omit the button entirely (the project's value is the case-study prose itself; not every project has an external destination). Do **not** derive the href from the legacy `src/data/projects.ts` shape — that file is being retired in favor of the `projects` content collection per the Wave 1 schema migration (audit `src/data/projects.ts` references in Pre-flight §2 and delete the file once the collection is the source of truth for `/projects` and the homepage).
- Body content through `.prose`.
- **Prev/Next strip at article foot.** Same `<ArticleNav />` component as post detail, sourced from the `sortProjects()` output (active first, then archive — prev/next walks the full sorted list so an active project's "Older" can be an archive entry without surprise). Omit cells at the ends as on post detail. Skip the strip entirely if `projects.length === 1`.
...
- **Option A (preferred, low-effort):** export a static 1200×630 PNG from the prototype's Paper hero (italic coral headline `I wonder what the machines think.` on `--bone` with a coral `§` mark top-left and `BRENNON WILLIAMS / VOL. 01 · Nº 03` caps strip top-right). Place at `public/og-image.png`. Reference from `BaseHead.astro` per the full meta-tag set below.
- **Option B (per-page dynamic):** Wire `@vercel/og` or `satori` in a future iteration to render per-post cards using the post title in Valkyrie B italic on bone. **Do not pursue in this PR** — adds a build dependency and runtime considerations that don't fit the chrome merge. Note it as a Wave-N follow-up if and when per-post sharing becomes a priority. (Per-post overrides via the schema `image:` field are already covered by Pre-flight §9 Option A — a small subset of dynamic without the build dep.)

**Full OG/Twitter meta-tag set in `BaseHead.astro` (pin all of these, not just `og:image`).** Cards visibly degrade on every major platform when these siblings are missing — LinkedIn falls back to first-paragraph scrape, Twitter renders a `summary` card instead of `summary_large_image`, Slack/Discord show no preview at all. Render the full set:

```astro
---
const socialImage = Astro.props.socialImage ?? { src: '/og-image.png', alt: 'Brennon Williams — I wonder what the machines think.' };
const absoluteImage = new URL(socialImage.src, Astro.site).toString();
const pageType = Astro.props.ogType ?? 'website';   // 'article' on post detail, 'website' elsewhere
const pageLocale = Astro.props.lang ?? 'en_US';     // OG uses underscore not hyphen
---
...
Post detail and project detail pass `ogType="article"` (plus `socialImage` from `post.data.image` if present, per Pre-flight §9 Option A). Every other page inherits the defaults — `og:type=website` and the site-wide card. The absolute-URL conversion is required: relative OG image paths fail silently on LinkedIn and most Slack-style unfurlers, even though Twitter and Facebook resolve them.
...
- `src/pages/404.mdx` — re-skin properly. Inside the full editorial chrome (side rails, topbar, nav) a blank "404" reads as broken. Give it: roman `§ 404` eyebrow, serif H1 ("This page didn't make the cut."), an italic serif sub-line, mono coord row, and two pill buttons ("Back to the workshop" → `/`, "Read the writing" → `/blog/`). Wrap in `EditorialLayout` with `mainWidth="prose"`. No marquee.
  - **`view-transition-name: brand-mark` is safe on /404.** The brand mark in `EdNav` declares this name on every page including /404; Astro's view-transitions engine morphs same-named elements between pages without requiring opt-in on the destination side. Since /404 also renders `EdNav`, the brand mark element exists at both ends of any nav and morphs cleanly. The hero portrait's `view-transition-name: hero-portrait` is exclusive to `/`'s hero — no other page claims it, so no collision risk. Verify by navigating `/blog` → (force a 404 via typing a bad URL) → back to `/` and confirming neither the brand mark nor any other element flashes unexpectedly.
  - **`noindex` on /404.** Emit `<meta name="robots" content="noindex,nofollow" />` from the 404 page (either as a `noindex` prop passed to `EditorialLayout` → `BaseHead`, or as an inline `<meta>` in the page itself). Without this, Google occasionally indexes the 404 body when it crawls a broken inbound link, and "This page didn't make the cut." shows up in SERPs as a real result. Same rule for any future under-construction or thank-you pages.
  - **Sitemap exclusion.** `@astrojs/sitemap` (already wired) auto-discovers `/404`. Add an explicit filter in `astro.config.ts` so it never appears in `sitemap-0.xml`: `sitemap({ filter: (page) => !page.endsWith('/404/') && !page.endsWith('/404') })`. Verify in §Verification step 15 by grepping the built `dist/sitemap-0.xml` for `404` — zero hits.
...
- **Component Architecture → Astro Components:** add `EditorialLayout.astro`, `EdSectionHeader.astro`, `ArticleNav.astro`. Re-skin `PovBlock.astro` note.
...
- **Styling Architecture → Token system:** document `tokens.css` categories (colors, fonts, **motion** [`--dur-*`, `--ease-*`], **z-index** [`--z-*`], **type/spacing scale** [`--fs-*`, `--space-*`]). Add the rule: "Never hardcode durations, easings, z-indexes, or hero-scale type clamps in component CSS — always reference a token."
- **Markdown plugins:** note that `astro.config.ts` wires `rehype-slug` + `rehype-autolink-headings` (Wave 1). Heading anchor markup is `<a class="heading-anchor">¶</a>`; style lives in `prose.css`.
- **MDX prose features:** call out that `prose.css` covers footnotes, figures with `<figcaption>`, GFM task lists, definition lists, opt-in drop cap (`p.lede`). The drop cap is the only feature requiring author opt-in.
...
- **Important Patterns:** add "Use `EditorialLayout` for every page, not `BaseLayout` directly", "Pass `lang` prop to `EditorialLayout` from any page that knows the content's language (post detail does)", "Sort projects via `sortProjects()` from `src/utils/projects.ts` — never iterate `getCollection('projects')` raw", and "Use `<ArticleNav prev next />` at the foot of any single-entry detail page that lives inside a sorted collection".
...
**Features explicitly NOT in scope (don't re-litigate).** Calling these out so a future agent doesn't wonder "should we add…":
- **Tags page (`/tags/[tag]`).** Posts have a `tag` field; it renders in the post meta row and on the homepage Writing card pills. No tag-index page. Add only when a second post shares a tag with a third — until then it's chrome over no data.
- **Related-posts strip.** `<ArticleNav prev/next />` is the only chronological article-foot navigation. A "related by tag/category" strip is omitted; the tag taxonomy is too sparse to drive meaningful relations.
- **Reading-progress indicator on post detail.** The right side rail is well-positioned to host one (rotated mono percentage), but the editorial framing favors stillness over instrumentation. Skipped intentionally.
- **Newsletter signup, comments, webmentions.** Out of scope. The site is publish-only.
- **Code-block copy button.** Punted to Wave-N — see §"Code-block copy button — explicit Wave-N punt".
- **Per-post OG cards (`@vercel/og` / `satori`).** Punted to Wave-N — see §OG card "Option B".
- **Command palette / `/` search.** The hero foot-row "Press / to search" prototype copy is replaced with non-actionable copy in §Hero. Restore only when the palette is wired.

### `<ArticleNav />` — new shared component

Lives at `src/components/ArticleNav.astro`. Two-column prev/next strip used at the foot of both post detail and project detail (see §Post detail and §Project detail in Wave 3 above). Props:

```ts
export interface ArticleNavProps {
  prev?: { href: string; title: string; eyebrow: string } | null;   // eyebrow e.g. 'OLDER'
  next?: { href: string; title: string; eyebrow: string } | null;   // 'NEWER'
}
```

If both `prev` and `next` are null (single-entry collection), the component renders nothing — the parent page can skip including it but the component is defensive. Each cell is a full `<a>` with `container-link` semantics (hover flips background to `var(--bone)`). Empty cells render an empty `<div>` to preserve the two-column grid (don't reflow when at the ends).
...
| `astro.config.ts` | Add Vite `define` for `__BUILD_HASH__`; configure Shiki `themes: { light, dark }` |
...
| `src/components/ArticleNav.astro` | **NEW** — prev/next strip at foot of post + project detail |
| `public/fonts/WOFF2/Inter Tight/*.woff2` | **NEW** — 6 self-hosted WOFF2 files (pre-Wave-1 asset task; see Pre-flight §5) |
| `package.json` | Add `rehype-slug` + `rehype-autolink-headings` deps (Wave 1 markdown plugin wiring) |
| `src/layouts/BaseLayout.astro` | **DELETE the hardcoded `<html lang="en">`** — `EditorialLayout` owns `<html>` now via its `lang` prop (default `'en-US'`). |
...
41. **`<html lang>` is dynamic.** View source on `/` (default `en-US`), then on a post that sets `lang: 'fr-FR'` in frontmatter (create a throwaway fixture if none exists). `<html lang>` must reflect the post's value. If both render `lang="en"`, the `EditorialLayout` `lang` prop wiring was skipped — `BaseLayout.astro:17`'s hardcoded `<html lang="en">` is still in effect.
42. **Markdown plugins wired.** Open `/projects/dotfiles` (or any post with `h2`/`h3` headings). Each heading must have an `id` attribute (`rehype-slug`) and an appended `<a class="heading-anchor" href="#…" aria-label="Permalink to this heading">¶</a>` (`rehype-autolink-headings`). Hover a heading on desktop: the pilcrow reveals in coral. On mobile: it's permanently at 0.5 opacity. If headings have no `id`, the plugin isn't wired in `astro.config.ts`.
43. **Heading anchor & deep-link works.** Click a `.heading-anchor` on a post; URL updates to `/posts/<slug>#<id>`; browser scrolls smoothly to the heading with it positioned *below* the sticky nav (validates `scroll-padding-top` + `scroll-behavior: smooth`). Reload the URL with the hash intact: page paints, then scrolls to the heading smoothly (not a hard jump — validates the native `scroll-behavior` gate landed).
44. **Native smooth-scroll respects reduced-motion.** Set `prefers-reduced-motion: reduce` in devtools, then click a heading anchor. The scroll should snap (instant), not animate. Flip back to no-preference: scrolls smoothly. Confirms the `@media (prefers-reduced-motion: no-preference)` wrap on `scroll-behavior` is in place.
45. **Archive WorkCard treatment.** Set one project to `status: 'archive'` in frontmatter, reload `/projects`. The archive card sits at `opacity: 0.62` next to active siblings; italic emphasis in its `name` reads in `--ink-mute` (not coral); the index cell shows a small `arch` tag in caps below the number; hover lifts opacity to ~0.85 but the card remains visibly quieted. Revert.
46. **Prev/Next nav at article foot.** Open a post that isn't first or last in `getPosts()` (middle of the list). Two cells render under the prose, separated by `--line` border-top: left `OLDER` with the older post's title, right `NEWER` with the newer one's. Click either; navigation succeeds. On the newest post the right cell is empty (no "nothing here" label); on the oldest, the left cell is empty. Repeat on `/projects/<slug>`.
47. **MDX prose features render.** Create a throwaway MDX fixture exercising: a footnote (`Text[^1]` + `[^1]: Note`), a `<figure>` with `<figcaption>`, a task list (`- [x] done`), a definition list. Open and verify: footnote ref is coral mono superscript with a hairline coral underline on hover; footnotes section sits under a `--line` border with mono back-arrow in coral; figcaption is caps + coral left rule; task-list checkboxes are square with coral fill when checked; `dt` is caps, `dd` is indented with `--line` left rule. Discard the fixture.
48. **Drop cap opt-in works.** Add `<p class="lede">…</p>` to any post and reload: first letter floats left in italic coral Valkyrie B at 4.5em. Remove the class: paragraph renders normally. Confirms drop cap is opt-in, not auto-applied.
49. **Motion / z-index tokens used, not hardcoded.** `rg -nE "(transition|animation):\\s*[^;]*\\b(180|200|360|60s)\\b" src/components src/layouts` and `rg -nE "z-index:\\s*\\d+" src/components src/layouts` after Wave 2. Expected hit count: zero (all references go through `var(--dur-*)` / `var(--ease-*)` / `var(--z-*)`). Any literal hit is a token-bypass that will silently drift the next time the scale changes.
50. **`<noscript>` ThemeToggle hidden.** Disable JS in devtools (`Cmd+Shift+P` → "Disable JavaScript") and reload `/`. The page renders fully styled; the `ThemeToggle` button is gone (no inert pill). Re-enable JS; toggle reappears.
51. **ArticleNav grid stable at ends.** Visit the newest post: right cell is empty but the grid stays 2-column (the empty `<div>` preserves layout). Visit the oldest: left cell empty, same. Cell content should never collapse the row.
